...
namespace TaskManagementSystem
{
    public class TaskManagementSystemDemo
    {
        public static void Main()
        {
            ...
            // Create tasks
            TaskM task1 = taskManagementSystem.CreateTask("Enhancement Task", "Launch New Feature",
                    "2024-02-15", TaskPriority.LOW, user1.GetId);
            TaskM subtask1 = taskManagementSystem.CreateTask("Enhancement sub task", "Design UI/UX",
                    "2024-02-14", TaskPriority.MEDIUM, user1.GetId);
            TaskM task2 = taskManagementSystem.CreateTask("Bug Fix Task", "Fix API Bug",
            ...
            // Search tasks
            List<TaskM> searchResults = taskManagementSystem.SearchTasks("Task", new SortByDueDate());
            Console.WriteLine("\nTasks with keyword Task:");
            foreach (TaskM task in searchResults)
            {
                Console.WriteLine(task.GetTitle);
            }

            // Filter tasks by status
            List<TaskM> filteredTasks = taskManagementSystem.ListTasksByStatus(TaskStat.TODO);
            Console.WriteLine("\nTODO Tasks:");
            foreach (TaskM task in filteredTasks)
            {
                Console.WriteLine(task.GetTitle);
            }
            ...
            List<TaskM> userTaskList = taskManagementSystem.ListTasksByUser(user2.GetId);
            Console.WriteLine($"\nTask for {user2.GetName}:");
            foreach (TaskM task in userTaskList)
            {
                Console.WriteLine(task.GetTitle);
            }
